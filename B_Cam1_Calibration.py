...
# =======================
# Paths (ANGEPASST FÜR ALLE CAM1 QUELLEN)
# =======================

# Hier alle Ordner auflisten, in denen Cam1 Bilder liegen könnten
IMAGE_DIRS = ["calib_1_2", "calib_1_3", "calib_single_1"]
OUTPUT_FILE = "mono_cam1_pinhole.pkl"
...
def load_images(folders):
    ...
    for folder in folders:
        ...
        # Suche gezielt nach cam1_*.png und cam1_*.jpg
        # Das verhindert, dass versehentlich cam2-Bilder aus den Stereo-Ordnern geladen werden
        pngs = glob.glob(os.path.join(folder, "cam1_*.png"))
        jpgs = glob.glob(os.path.join(folder, "cam1_*.jpg"))
        all_files.extend(pngs + jpgs)
        
    files = sorted(all_files)
    if not files:
        raise RuntimeError(f"❌ Keine Cam1-Bilder in den Verzeichnissen {folders} gefunden.")
    
    print(f"✅ Insgesamt {len(files)} Bilder für Cam1 gefunden.")
    return files
...
def calibrate(images):
    ...
    K = np.eye(3, dtype=np.float64) # erzeugt 3x3 Einheitsmatrix, Startwert für Kameramatrix, enthält Brennweite und optischen Mittelpunkt
    D = np.zeros((5, 1), dtype=np.float64) # erzeugt Vektor mit 5 Nullen, Startwert für Distorsionskoeffizienten
    
    rms, K, D, rvecs, tvecs = cv2.calibrateCamera( # Berechnet, wie die 3D-Welt auf den 2D-Sensor projeziert wird
