...
# Hier alle Ordner auflisten, in denen Cam2 Bilder liegen könnten
IMAGE_DIRS = ["calib_1_2", "calib_single_2"]
OUTPUT_FILE = "mono_cam2_pinhole.pkl"
...
def load_images(folders):
    ...
    for folder in folders:
        ...
        # Suche gezielt nach cam2_*.png und cam2_*.jpg
        # Das verhindert, dass versehentlich cam2-Bilder aus den Stereo-Ordnern geladen werden
        pngs = glob.glob(os.path.join(folder, "cam2_*.png"))
        jpgs = glob.glob(os.path.join(folder, "cam2_*.jpg"))
        all_files.extend(pngs + jpgs)
        
    files = sorted(all_files)
    if not files:
        raise RuntimeError(f"❌ Keine Cam2-Bilder in den Verzeichnissen {folders} gefunden.")
    
    print(f"✅ Insgesamt {len(files)} Bilder für Cam2 gefunden.")
    return files
